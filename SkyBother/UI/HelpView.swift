...
enum HelpContent {
    static let topics: [HelpTopic] = [
        ...
        HelpTopic(title: "Score Breakdown", systemImage: "list.bullet.rectangle", sections: [
            ...
            HelpSection(heading: "In Your Frame", body: "The framing preview shows real sky, not a drawing: a cutout of the Digitized Sky Survey centred on the target, at exactly your rig\u{2019}s field of view, with the frame outline over it. The dashed ellipse is the target\u{2019}s *catalogued* extent, which for many objects is far smaller than the part worth photographing \u{2014} an open cluster sitting inside a nebula is catalogued as the cluster, and the nebula is what you came for. Seeing how far the real thing runs past that outline is the point of showing actual pixels. Images are cached on disk, so a target you have looked at once works offline afterwards; with no network and nothing cached, the preview falls back to the schematic star field it always used. DSS covers the whole sky, which is why it is used in preference to deeper surveys that leave gaps."),
