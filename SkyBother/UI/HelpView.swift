...
enum HelpContent {
    static let topics: [HelpTopic] = [
        ...
        HelpTopic(title: "Sky View", systemImage: "globe", sections: [
            ...
            HelpSection(heading: "What's Drawn on It", body: "The dome is the real night sky, turning with the time slider: the Milky Way, its dust lanes and the stars are all there, so there are no separate dots or bands to read. Twilight and moonlight are laid over it the way they are outside \u{2014} the sky brightens towards the Sun and the horizon at dusk and dawn, and a bright Moon lifts the whole sky and washes the Milky Way out first, leaving only the brighter stars. Only your selected target is marked, so the view is always about what you are about to point at.", swatches: [
                HelpSwatch(color: Palette.go, label: "The selected target \u{2014} green corner brackets with its name beneath, around your rig's true-size frame. The same green as the rig frame in the sky browser"),
                HelpSwatch(color: Palette.moonlight, label: "The Moon \u{2014} a plain disc, sized to its real apparent diameter for tonight's actual distance, only drawn while it's above the horizon"),
            ]),
            HelpSection(heading: "The Selected Target's Path", body: "Once a target is selected, its full day-long track is traced as a closed loop around the sky — a real fixed-coordinate target genuinely circles the pole once a day, and drawing the whole loop instead of stopping it at dusk and dawn is what makes that shape read as a real orbit rather than an arc that just happens to begin and end at nightfall. The portion above the horizon draws as a solid purple line; the rest of the loop, while it's below the horizon and out of reach, continues on as a faint one instead of vanishing, so the ring stays visibly whole. Wherever the above-horizon portion passes through a zenith-risk span (see below), it switches to amber instead of purple, the same amber used for warnings throughout the app. The target's brackets always sit on its own path, at whatever point corresponds to the current scrubbed time."),
            ...
            HelpSection(heading: "Rig and Roll", body: "Above the dome, a rig picker previews any preset or saved rig here without changing the active one used for scoring, and a roll slider turns the frame, since its on-sky orientation at imaging time isn\u{2019}t something the app can know on its own. The frame is only drawn when it clears the horizon entirely, and not at all within 2\u{b0} of the zenith, where this flat view can\u{2019}t draw it meaningfully; the brackets still mark the target either way. On an alt-az rig the frame visibly rotates as you scrub through the night \u{2014} that\u{2019}s real field rotation, the same effect behind Zenith Risk. On an equatorial rig it holds one orientation relative to the stars all night, same as the real mount does."),
            HelpSection(heading: "The Readouts Beneath the Scrubber", body: "As you drag the time slider, the line beneath it reports the selected target\u{2019}s live altitude and compass direction, and the line under that the rig\u{2019}s field and where the frame is pointing."),
            HelpSection(heading: "Playing Through the Night", body: "The round button beside the slider plays the whole chart window forward on its own, about 25 real seconds per evening, rather than dragging by hand. If Tonight's Plan has anything in it, two chips appear on the readout line to choose what playback does with the selection: \"Track Selected\" holds whatever's currently selected fixed while time advances, and \"Cycle Plan\" hands the selection off from one planned target to the next as playback crosses into each one's own scheduled window, so the detail pane on the right walks through the whole session with it. Which one is picked resets every time the selection itself changes: choosing something from tonight's plan — or nothing at all — defaults to Cycle Plan, while choosing anything else (browsing the full catalogue, say) switches to Track Selected automatically, since there'd be nothing of the plan left to hand off to otherwise. Either chip can still be tapped directly to override that default until the next selection change. Dragging the slider by hand always pauses playback rather than fighting it.")
        ]),
        ...
        HelpTopic(title: "Tonight's Plan", systemImage: "list.number", sections: [
            ...
            HelpSection(heading: "Manual or Suggested", body: "The badge beside the section title says whose plan you are looking at. \u{201c}Suggested\u{201d} is the app\u{2019}s own, rebuilt whenever the forecast or your settings change. \u{201c}Manual\u{201d} means you have edited that night: it then stays exactly as you left it and nothing re-plans it, A night only becomes manual when you change its plan and press \u{201c}Done\u{201d}; opening the editor and closing it again leaves a suggestion a suggestion. \u{201c}Reset manual plan\u{201d} asks first, then removes your version and hands the night back to the app\u{2019}s current suggestion \u{2014} with an Undo until you next edit a plan. Plans are saved per night and survive quitting; nights that have already passed are dropped on the next launch, since this is a planner rather than a logbook."),
            HelpSection(heading: "Editing the Plan", body: "\u{201c}Edit plan,\u{201d} beside the section title, turns the strip into something you build on \u{2014} it takes on the accent-coloured border while it\u{2019}s live. It starts from whatever the app was already suggesting, since the scheduler usually has the shape of the night about right and the edits worth making are moving one block and splitting another. Drag a block along the night to move it; drag either end to change how long you spend there. Every edit snaps to the nearest five minutes so times stay readable. Nothing is saved while you edit. \u{201c}Clear\u{201d} empties the draft; \u{201c}Cancel\u{201d} (or Esc) throws the draft away and puts back what was there. \u{201c}Done\u{201d} saves your changes as this night\u{2019}s manual plan, which survives quitting the app; with no changes the button reads \u{201c}Close\u{201d} and saves nothing."),
            ...
            HelpSection(heading: "Longer Integration or More Targets", body: "Settings \u{2192} Planning chooses which way the suggested plan leans when the night is shorter than the targets that want it. \u{201c}Longer integration\u{201d} hands no target more than your full Integration Goal before the others get a turn, and won\u{2019}t suggest anything under a third of that at all \u{2014} a twenty-minute slot is gone by the time the mount has slewed and refocused, so offering one would contradict the setting. \u{201c}More targets\u{201d} halves the cap and drops that floor back to twenty minutes, so roughly twice as many fit and the short windows early in the night get used. Either way, time nobody else wants is still given back afterwards \u{2014} a night with one real target on it keeps the whole night. This only shapes what the app suggests: a plan you have edited by hand is yours, and changing this never rearranges it."),
        ...
        HelpTopic(title: "\"In Your Frame\"", systemImage: "camera.viewfinder", sections: [
            ...
            HelpSection(heading: "Reading It", body: "The picture is real sky: a Digitized Sky Survey cutout centred on the target, at exactly your rig\u{2019}s field of view. The rectangle over it is your frame. Solid green means the target fits comfortably in a single frame. Dashed amber means it\u{2019}s oversized and would need a mosaic, or is being shown anyway because oversized targets are allowed in Settings. The number in the corner is your configured field of view in degrees, and the small cross marks the target\u{2019}s catalogued position \u{2014} the survey is shallow, so a faint target can be hard to make out. Its catalogued size, pixel scale and the photo credit are in the lines underneath."),
            HelpSection(heading: "Before the Picture Arrives", body: "The panel shows your frame over an empty sky and says it\u{2019}s fetching, then fills in when the cutout lands \u{2014} usually a second or two, and instantly for anything looked at before, since cutouts are cached on disk. If it can\u{2019}t be fetched at all, offline most likely, it says so rather than inventing something. Earlier versions drew a made-up star field with the catalogued size as an ellipse over it, which looked like a finished picture of something it wasn\u{2019}t, and disagreed with the real sky that replaced it a moment later."),
            HelpSection(heading: "Rig Presets", body: "The presets in Settings \u{2192} Equipment use published optical specs and the standard dimensions of each model\u{2019}s sensor. Check them against your own unit \u{2014} every number is editable, and a rig you save keeps your changes.")
        ]),
        ...
        HelpTopic(title: "Score Breakdown", systemImage: "list.bullet.rectangle", sections: [
            ...
            HelpSection(heading: "In Your Frame", body: "The framing preview shows real sky, not a drawing: a cutout of the Digitized Sky Survey centred on the target, at exactly your rig\u{2019}s field of view, with the frame outline over it. Seeing the real thing is the point: a catalogued extent describes something far smaller than the part worth photographing for many objects \u{2014} an open cluster sitting inside a nebula is catalogued as the cluster, and the nebula is what you came for. Images are cached on disk, so a target you have looked at once works offline afterwards; until one arrives the panel shows your frame over an empty sky, and says so if it can\u{2019}t be fetched. DSS covers the whole sky, which is why it is used in preference to deeper surveys that leave gaps."),
            ...
            HelpSection(heading: "Framing", body: "Full marks go to a target filling 30 to 80 percent of the frame\u{2019}s long side. A target too small for your rig to resolve \u{2014} under about 40 pixels across, which for a small smart telescope means anything under about a minute and a half of arc \u{2014} scores near zero here and lands in Poor however bright it is: there is no shape to record, only a fatter star. The same object scores properly at a focal length that does resolve it, since this is judged in pixels. Smaller under-uses the sensor; larger needs a mosaic, which costs a little if your rig supports mosaicking and a lot if it doesn't.")
        ]),
        ...
        HelpTopic(title: "Cloud, Moon & Weather Numbers", systemImage: "cloud.moon", sections: [
            ...
            HelpSection(heading: "Seeing", body: "Seeing is estimated from surface wind gusts. Real seeing depends on jet stream conditions, which no free forecast exposes — treat this as a hint, not a measurement."),
            HelpSection(heading: "Estimated Dew Risk", body: "Dew risk is rated from the forecast spread between air temperature and dew point: more than 10°F is Low, 6–10°F Moderate, 3–6°F High, and 3°F or less Very High. It doesn't wait for the spread to reach zero, because a telescope under a clear sky radiates heat away and cools below the air around it — its optics can dew over while the reported temperature is still several degrees above the dew point. So when the sky is mostly clear and the wind is under about 5 mph, the rating goes up one step. Cloud (which sends heat back down) or a breeze (which keeps the tube at air temperature) withholds that step, but never lowers a Very High spread. The rating shown is the worst hour of your session — Tonight's Plan from its first block to its last, or astronomical darkness without a plan — with the tightest spread and when it comes, and the bar under the night chart shows how it changes through the night. It's an estimate from the forecast, not a prediction that dew will or won't form.", swatches: [
                HelpSwatch(color: Palette.dewRisk(.low), label: "Low — dew heater probably unnecessary"),
                HelpSwatch(color: Palette.dewRisk(.moderate), label: "Moderate — monitor optics; dew protection may be useful"),
        ...
        HelpTopic(title: "Bortle Class & Your Site", systemImage: "mappin.and.ellipse", sections: [
            ...
            HelpSection(heading: "Darker Sky", body: "Looks for somewhere the sky should be noticeably darker than at your site. Bortle classes here, yours and each spot\u{2019}s, are estimated from NASA satellite measurements of night lights. It shows the place's estimated Bortle class next to the same estimate for your site, and how many targets would score Good or better from there tonight compared with from here. Use This Spot switches planning to it and saves it next to your current site; Back to… switches home again. If the Bortle class you set for your site is two or more classes off the satellite estimate, the panel says so and offers to update it — the comparison is only as fair as that setting. It assumes the same blocked horizon as your site — if the spot is more open, lower it in Settings. And it can't know whether a park is open or safe after dark, so check before you go."),
        ...
        HelpTopic(title: "Planning Settings", systemImage: "slider.horizontal.3", sections: [
            ...
            HelpSection(heading: "UI Scale", body: "Under the night list. With Auto on, the whole interface sizes itself to the window: a comfortable size for its width \u{2014} about 120% filling a laptop screen, nearer 145% filling a 4K one \u{2014} or smaller if that\u{2019}s what it takes for the night rows, Tonight\u{2019}s Plan and the target list to stay on one line with nothing cut off. It re-fits whenever the window changes size. The night\u{2019}s statistics and the chart key wrap whole items onto a second row rather than shrinking everything to fit on one. Turn Auto off to set the size yourself with the slider, which starts from wherever Auto had got to."),
        ...
        HelpTopic(title: "Data Sources & Accuracy", systemImage: "antenna.radiowaves.left.and.right", sections: [
            ...
            HelpSection(heading: "Weather", body: "Cloud cover (split into low, mid and high layers), dew point, humidity, wind and gusts all come from Open-Meteo — a free forecast API that needs no account and no API key. Across the US and southern Canada the numbers are NOAA\u{2019}s National Blend of Models, the weighted, bias-corrected blend of every major model that the National Weather Service\u{2019}s own forecast starts from, rather than a single model that can be the odd one out on a night the models disagree. The Blend gives total cloud but no layers, so each hour\u{2019}s low/mid/high mix is taken from Open-Meteo\u{2019}s regular forecast and scaled to the Blend\u{2019}s total. Outside its area, and past its range of about 11 days, the regular forecast is used on its own. One request is made per location each time you refresh; the age of the data currently in use is shown in the sidebar footer. If Open-Meteo can't be reached, the app quietly falls back to MET Norway (also free, also keyless), and the footer says \"backup source\" so a lower-fidelity forecast is never mistaken for the primary one — MET Norway doesn't publish wind gusts or a precipitation probability outside the Nordics, so those two figures become estimates in that case. To avoid hammering either service, automatic refreshes (app launch, changing a setting) are limited to once an hour; the Refresh button and Cmd-R always fetch immediately regardless."),
            ...
            HelpSection(heading: "The Moon", body: "The Moon drawn when you click it in a night\u{2019}s header is NASA\u{2019}s Scientific Visualization Studio CGI Moon Kit colour map, made from Lunar Reconnaissance Orbiter Camera data, wrapped onto a sphere and lit from the Sun\u{2019}s direction at the moment the Moon stands highest that night, turned the way it appears from your site. Libration and the small tilt of the Moon\u{2019}s axis are left out."),
            HelpSection(heading: "The Sky View's Star Map", body: "The sky in Sky View is NASA\u{2019}s Deep Star Maps 2020 from the Goddard Scientific Visualization Studio (svs.gsfc.nasa.gov/4851), rendered from the positions, brightnesses and colours of 1.7 billion stars in the Gaia DR2 (ESA/Gaia/DPAC), Hipparcos-2 and Tycho-2 catalogues. It ships inside the app, so the dome works offline. Twilight and moonlight over it are modelled on your Mac from the Sun and Moon positions above \u{2014} an impression of how bright the sky is, not a photometric prediction.")
        ])
    ]
}
